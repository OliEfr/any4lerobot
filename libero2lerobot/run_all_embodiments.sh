...
#
# Phase 1: libero_90 per embodiment, fully sequential (raw output is ~250 GB each,
#          so only one may exist at a time).
# Phase 2: the four small suites per embodiment with the regenerations running in
#          parallel (~30 GB raw each), then sequential convert+push per suite.
#
# Successes go to OliverHausdoerfer/<suite>_<robot>_additionalCams, failed replays
...
regen() { # args: robot suite tag
    $PYTHON libero_utils/regenerate_libero_dataset.py \
        ...
        --libero_target_dir "$BASE/libero_datasets_regenerated/${3}_additionalCams" \
...
publish() { # args: robot_lc tag
    ...
    local RAW="$BASE/libero_datasets_regenerated/${TAG}_additionalCams"
    local RAW_FAIL="${RAW}_failures"
    local REPO="OliverHausdoerfer/${TAG}_additionalCams"

    if convert_push "$RAW" "$REPO" "$robot_lc" "$LOGDIR/${TAG}_convert.log"; then
        log "convert+push OK $TAG -> $REPO"
        rm -rf "$RAW" "$BASE/lerobot_datasets/${TAG}_additionalCams_lerobot"
    else
    ...
    if [ -d "$RAW_FAIL" ] && [ -n "$(ls -A "$RAW_FAIL" 2>/dev/null)" ]; then
        if convert_push "$RAW_FAIL" "${REPO}_failures" "$robot_lc" "$LOGDIR/${TAG}_convert_failures.log"; then
            ...
            rm -rf "$RAW_FAIL" "$BASE/lerobot_datasets/${TAG}_additionalCams_failures_lerobot"
        else
...
for ROBOT in "${ROBOTS[@]}"; do
    ...
    if regen "$ROBOT" libero_90 "$TAG"; then
        ...
        rm -rf "$BASE/libero_datasets_regenerated/${TAG}_additionalCams" \
               "$BASE/libero_datasets_regenerated/${TAG}_additionalCams_failures"
        continue
    fi
    publish "$robot_lc" "$TAG"
done

# ---------- Phase 2: small suites, regen 4x parallel per robot ----------
for ROBOT in "${ROBOTS[@]}"; do
    robot_lc=$(echo "$ROBOT" | tr '[:upper:]' '[:lower:]')
